<?php
namespace OneClickModal\Classes;
...
class ModalWindow{
	...
	public function modal_preview( $content, $element ){
		...
		$content = '<# if ( settings.show_in_modal ) {
			...
			<div id="modal_{{id}}' .get_the_ID(). '" class="ocmw-modal edit-modal">
				<div class="ocmw-modal-content"> 
					<div class="ocmw-modal-inner"> 
					<span onClick="closeModal(\'{{id}}' .get_the_ID(). '\')" class="close-modal">X</span>
    ...
    function get_random_string($length = 15) {
	...
	public function before_element_render( $settings, $wg_id  ){
		if( ! isset( $settings['show_in_modal'] ) || ! $settings['show_in_modal'] ){
			...
			$modal_num = $this->get_random_string();
			...
			$before .= '<div id="modal_' .$modal_num. '" class="ocmw-modal edit-modal">
					<div class="ocmw-modal-content"> 
						<div class="ocmw-modal-inner"> 
						<span data-modal="' .$modal_num. '" class="close-modal">X</span>
	...
	public function modal_controls( $element, $args ) { 
		$element->start_controls_section(
			'modal_section',
			[
				'label' => __( 'Modal Window', OCMW_NS ),
				'tab' => \Elementor\Controls_Manager::TAB_CONTENT,
			]
		);

/* 		$element->add_control(
			'modal_ocmwture_notice',
			[
				'show_label' => false,
				'type' => \Elementor\Controls_Manager::RAW_HTML,
				'raw' => sprintf( '<h2>%s</h2></br><p>%s</p>', __( OCMW_TITLE.' Feature', OCMW_NS ), __( 'Please note: this ocmwture will moved to a dedicated plugin on April 20th, 2022. We will add a download link soon.', OCMW_NS ) ),
			]
		); */
		...
		$element->add_control(
			'show_in_modal',
			[
				'label' => __( 'Show in Modal', OCMW_NS ),
				'type' => \Elementor\Controls_Manager::SWITCHER,
				'label_on' => __( 'Yes', OCMW_NS ),
				'label_off' => __( 'No',OCMW_NS ),
				'return_value' => 'true',
			]
		);
			
		$default_text = __( 'Open Modal', OCMW_NS );

		$element->add_control(
			'modal_button_text',
			[
				'label' => __( 'Modal Button Text', OCMW_NS ),
		...
		$element->add_control(
			'modal_button_icon',
			[
				'label' => __( 'Modal Button Icon', OCMW_NS ),
		...
		$element->add_control(
			...
			[
				...
				'type' => \Elementor\Controls_Manager::COLOR,
				'selectors' => [
					'{{WRAPPER}} .edit-modal .ocmw-modal-content' => 'background-color: {{VALUE}};',
				],
			]
		);
		...
		$element->add_responsive_control(
			...
			[
				...
				'selectors' => [
					'{{WRAPPER}} .edit-modal .ocmw-modal-content' => 'width: {{VALUE}}%',
				],
			]
		);
		...
		$element->add_responsive_control(
			...
			[
				...
				'selectors' => [
					'{{WRAPPER}} .edit-modal .ocmw-modal-content .ocmw-modal-inner' => 'width: {{VALUE}}%',
				],
			]
		);
		...
		$element->add_responsive_control(
			...
			[
				...
				'selectors' => [
					'{{WRAPPER}} .edit-modal .ocmw-modal-content' => 'justify-content: {{VALUE}}',
				],
			]
		);
    ...
    public function frontend_scripts(){
        ...
        wp_enqueue_style( 'ocmw-modal-window', OCMW_URL . 'assets/css/modal'.$min.'.css', array(), OCMW_VERSION );	            
        wp_enqueue_script( 'ocmw-modal-window', OCMW_URL . 'assets/js/modal'.$min.'.js', array( 'jquery' ), OCMW_VERSION );
    }
